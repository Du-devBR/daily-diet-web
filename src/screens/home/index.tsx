    ...
    <div className="px-6 pb-6 md:max-w-5xl m-auto">
      ...
      <main className=" mt-10 w-full">
        <div className="w-full">
          <h2 className=" text-bodyM text-BaseGray100 font-nunito">
            Refeições
          </h2>
          <Link to={"/create"} className=" active-solid-button w-full mt-2">
        ...
        <section className=" mt-8">
          {loading ? (
            <Skeleton
              className="flex gap-3 items-center py-4 px-3 w-full border border-BaseGray500 rounded-md"
              count={10}
            />
          ) : (
            Object.keys(groupCreatedat).map((date) => (
              <div className=" mt-8" key={date}>
                <h2 className=" text-titleS text-BaseGray100 font-nunito mb-2">
                  {formattedDate(date)}
                </h2>
